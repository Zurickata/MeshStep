...
def combine_historial(name,LR,vtkq,vtkc,vtkr,vtks):
...
def combine_historial_borde(name,LR,vtkq,vtkc,vtkr,vtks):
    output_file = "historial_completo_new.txt"
    input_files = [
        "movimientos1_new.txt",
        "movimientos2_new.txt",
...
def crear_historial(name,nivel_refinamiento,tipo="completo", input_dir="."):
    ...
    if tipo == "borde":
        combine_historial_borde(name,nivel_refinamiento,f"{name}_quads.vtk",f"{name}_closeto.vtk",f"{name}_remSur.vtk",f"{name}_shrimk.vtk")
    else:
        combine_historial(name,nivel_refinamiento,f"{name}_quads.vtk",f"{name}_closeto.vtk",f"{name}_remSur.vtk",f"{name}_shrimk.vtk")
