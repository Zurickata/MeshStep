...
# def subdividir_completo(name,nivel_refinamiento):
#     for x in range(nivel_refinamiento):
#         subdividir_malla_por_caras(
#             f"{name}_grid.vtk", #entrada
#             f"{name}_{x+1}_subdividida.vtk", #salida
#             subdivs=2**(x+1)
#         )

def subdividir_completo(name, nivel_refinamiento, input_dir="."):
    # Construimos la ruta completa del archivo de entrada
    input_file_path = os.path.join(input_dir, f"{name}_grid.vtk")

    for x in range(nivel_refinamiento):
        subdividir_malla_por_caras(
            input_file_path, 
            f"{name}_{x+1}_subdividida.vtk",
            subdivs=2**(x+1)
        )

# subdividir_completo("a_debug_5", 5, "../outputs")